...
class new_reg_branch_laplace(nn.Module):
    """Regression head that predicts point means and Laplace scale params jointly."""
...
class MapDetectorHead(nn.Module):
    ...
    def _init_branch(self,):
        ...
        if self.loss_reg.__class__.__name__ == 'LinesL1Loss':
        ...
        elif self.loss_reg.__class__.__name__ == 'PtsNLLLoss':
            reg_branch = new_reg_branch_laplace()
        else:
            raise NotImplementedError(
                f"Unsupported loss_reg type: {self.loss_reg.__class__.__name__}")
    ...
    def forward_train(self, bev_features, img_metas, gts):
        ...
        inter_references = []
        betas_per_layer = []
        for ref in final_inter_references:
            inter_references.append(ref[:, :, :, 0:2])
            if self.loss_reg.__class__.__name__ == 'LinesL1Loss':
                betas_per_layer.append(ref[:, :, :, 0:2] * 0.0 + 1.0)
            elif self.loss_reg.__class__.__name__ == 'PtsNLLLoss':
                betas_per_layer.append(ref[:, :, :, 2:4])
            else:
                raise NotImplementedError

        outputs = []
        for i, (queries) in enumerate(inter_queries):
            reg_points = inter_references[i]
            beta = betas_per_layer[i]
            bs = reg_points.shape[0]
            reg_points = reg_points.contiguous().view(bs, -1, 2 * self.num_points)
            beta = beta.contiguous().view(bs, -1, 2 * self.num_points)

            scores = self.cls_branches[i](queries)
    ...
    def forward_test(self, bev_features, img_metas):
        ...
        for i, (queries) in enumerate(inter_queries):
            ...
            for j in range(len(scores)):
                ...
                scores_list.append(scores[j])
                betas_list.append(beta[j])
                prop_mask = scores.new_ones((len(scores[j]),), dtype=torch.bool)
    ...
    def loss_single(self,
        ...
        betas = torch.cat(preds['betas'], dim=0)
        ...
        if self.loss_reg.__class__.__name__ == 'LinesL1Loss':
            loss_reg = self.loss_reg(pred_lines, gt_lines, line_weights, avg_factor=num_total_pos)
        elif self.loss_reg.__class__.__name__ == 'PtsNLLLoss':
            loss_reg = self.loss_reg(
                torch.cat([pred_lines, betas], dim=-1), gt_lines, line_weights, avg_factor=num_total_pos)
        else:
            raise NotImplementedError
