...
class MapTransformerDecoder_new(BaseModule):
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reg_branches[0].__class__.__name__ == 'new_reg_branch_laplace':
                reg_output = reg_branches[lid](output.permute(1, 0, 2))
                reg_points = reg_output[:, :, 0:40]
                reg_betas = reg_output[:, :, 40:80]
                bs, num_queries, num_points2 = reg_points.shape
                reg_points = reg_points.view(bs, num_queries, num_points2 // 2, 2)
                reg_betas = reg_betas.view(bs, num_queries, num_points2 // 2, 2)
            else:
            ...
            if reg_branches[0].__class__.__name__ == 'new_reg_branch_laplace':
                final_reference_points = torch.cat([reference_points, reg_betas], dim=-1)
                if self.return_intermediate:
                    intermediate.append(output.permute(1, 0, 2))
                    intermediate_reference_points.append(
                        torch.cat([new_reference_points, reg_betas], dim=-1))
            else:
                final_reference_points = reference_points.clone()
                if self.return_intermediate:
                    intermediate.append(output.permute(1, 0, 2))
        ...
        return output, final_reference_points
