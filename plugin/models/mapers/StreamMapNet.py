...
from plugin.models.aid.pipeline import AID_PIPELINES
...
class StreamMapNet(BaseMapper):
    ...
    def __init__(self,
                 ...
                 streaming_cfg=dict(),
                 pretrained=None,
                 aid_pipeline: Optional[dict] = None,
                 trainable_prefixes: Optional[Sequence[str]] = None,
                 **kwargs):
        super().__init__()
        ...
        self.aid = None
        if aid_pipeline is not None:
            self.aid = build_from_cfg(aid_pipeline, AID_PIPELINES)
        ...
        self._freeze_by_prefixes(trainable_prefixes, set_bn_eval=True)

    def _freeze_by_prefixes(self, prefixes: Optional[Sequence[str]] = None, set_bn_eval: bool = True):
        """Freeze all parameters except those whose name matches one of the given prefixes."""
        if prefixes is None:
            return

        for name, p in self.named_parameters():
            p.requires_grad = any(
                name.startswith(pref) or ('.' + pref + '.') in name
    ...
    def forward_train(self, img, vectors, aerial_img=None, points=None, img_metas=None, **kwargs):
        ...
        if self.aid is not None:
            inputs = {'aerial_img': aerial_img}
            _bev_feats, loss_bev = self.aid(_bev_feats, inputs, return_aux=True)
    ...
    def forward_test(self, img, aerial_img=None, points=None, img_metas=None, **kwargs):
        ...
        if self.aid is not None:
            inputs = {'aerial_img': aerial_img}
            _bev_feats = self.aid(_bev_feats, inputs)
    ...
    def batch_data(self, vectors, imgs, img_metas, device, points=None):
        ...
        assert len(valid_idx) == bs, \
            f"len(valid_idx)={len(valid_idx)}, expected={bs}, valid_idx={valid_idx}, \n {img_metas}"
