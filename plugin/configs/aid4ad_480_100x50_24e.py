...
model = dict(
    ...
    aid_pipeline = dict(
        ...
        fuser=dict(
            type='ConvFuser',
            in_ch=2 * bev_embed_dims,
            out_ch=bev_embed_dims,
        ),
    ),
    backbone_cfg=dict(
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'aid.encoder': dict(lr_mult=0.1, decay_mult=1.0),
            'aid.down': dict(lr_mult=1.0, decay_mult=1.0),
            'aid.fuser': dict(lr_mult=0.5, decay_mult=1.0),
        }),
    weight_decay=1e-2)
