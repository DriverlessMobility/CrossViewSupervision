...
aerial_img_norm_cfg = dict(
    mean=[98.8, 102.3, 104.3], 
    std=[60.4, 57.7, 55.1],    
    to_rgb=False)
...
num_gpus = 1
batch_size = 4
num_iters_per_epoch = 27807 // (num_gpus * batch_size)
num_epochs = 24
...
model = dict(
    ...
    bev_w=bev_w,
    aid_pipeline = dict(
        type='CrossViewSupervisionPipeline',
        crop_size=None,
        fusion_mode='ego',
        loss_bev_weight=70,
        loss_bev_norm=True,
        loss_bev_function='MSE',
        encoder=dict(
            type='ResNetUNet',
            outC=64,
        ),
        adapter_bev_teaching=dict(
            type='AffineAdapter',
            channels=bev_embed_dims,
        ),
        downsampler=dict(
            type='DownsampleCNN',
            hidden_dim=bev_embed_dims // 2,
            use_gn=False,
        ),
    ),
    backbone_cfg=dict(
...
train_pipeline = [
    ...
    dict(type='PhotoMetricDistortionAerialImage'),    
    dict(type='ResizeMultiViewImages',
         size=img_size, # H, W
         change_intrinsics=True,
         ),
    dict(type='Normalize3D', **img_norm_cfg),
    dict(type='NormalizeAID', **aerial_img_norm_cfg),
...
annot_dir = './datasets/nuScenes/nuscenes_map_infos_'
aerial_image_dir = './datasets/AID4AD_ego_referenced_100x50'
...
optimizer = dict(
    type='AdamW',
    lr=1.25e-4 * (num_gpus * batch_size) / (1 * 4),
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone': dict(lr_mult=0.1),
            'aid.encoder': dict(lr_mult=0.0, decay_mult=1.0),
            'aid.down': dict(lr_mult=0.0, decay_mult=1.0),
        }),
    weight_decay=1e-2)
