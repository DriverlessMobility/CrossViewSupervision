...
class NormalizeAID(object):
    ...
    def __call__(self, results):
        results['aerial_img'] = mmcv.imnormalize(
            results['aerial_img'], self.mean, self.std, self.to_rgb)
...
@PIPELINES.register_module()
class PhotoMetricDistortionAerialImage:
    """Apply photometric distortion to the aerial image."""
    ...
    def __call__(self, results):
        img = results['aerial_img']
        assert img.dtype == np.float32, \
            'PhotoMetricDistortionAerialImage needs float32 input. Set to_float32=True when loading.'
        ...
        if mode == 1 and random.randint(2):
            img *= random.uniform(self.contrast_lower, self.contrast_upper)
